...
async function writePlayerToDB(playerSummary, response) {
  ...
  let espnRushingCategory = response.data.categories.filter((statCategory) => {
    return statCategory.name === "rushing";
  })[0];
  let espnScoringCategory = response.data.categories.filter((statCategory) => {
    return statCategory.name === "scoring";
  })[0];
  let espnPassingCategory = response.data.categories.filter((statCategory) => {
    return statCategory.name === "passing";
  })[0];
  let espnReceivingCategory = response.data.categories.filter(
    (statCategory) => {
      return statCategory.name === "receiving";
    }
  )[0];
  let espnResponseCategories = [
    espnRushingCategory,
    espnReceivingCategory,
    espnPassingCategory,
    espnScoringCategory,
  ];
